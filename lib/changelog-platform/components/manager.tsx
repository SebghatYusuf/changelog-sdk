...
export default function ChangelogManager({ params, searchParams, basePath }: ChangelogManagerProps) {
  ...
  // Route logic
  if (route === 'admin') {
    content = <AdminPortalRoute section={adminSection} editId={adminEditId} preset={searchParams?.preset} basePath={resolvedBasePath} />
  } else if (route === 'login') {
    content = <LoginRoute basePath={resolvedBasePath} />
  } else if (route === 'preview') {
    content = <PreviewRoute token={params?.route?.[1]} basePath={resolvedBasePath} />
  } else if (route) {
    content = <DetailRoute slug={route} basePath={resolvedBasePath} />
  } else {
    // Default: public feed
    content = <PublicFeedRoute searchParams={searchParams} basePath={resolvedBasePath} />
  }
 ...
 * Public Feed Route
 */
function PublicFeedRoute({ searchParams, basePath }: { searchParams?: { page?: string; tags?: string; search?: string }; basePath: string }) {
...
function PreviewRoute({ token, basePath }: { token?: string; basePath: string }) {
  return (
    <main className="cl-root cl-section cl-feed-screen">
      <Suspense fallback={<LoadingFallback />}>
        <ChangelogDetail previewToken={token} basePath={basePath} />
      </Suspense>
    </main>
  )
}
 ...
 * Admin Portal Route
 */
function AdminPortalRoute({ section, editId, preset, basePath }: { section?: string; editId?: string; preset?: string; basePath: string }) {
  return (
    ...
    return (
      <div className="cl-card cl-auth-guard-card">
        ...
        <div className="cl-card-content">
          ...
          <a href={joinPath(basePath, 'login')} className="cl-btn cl-btn-primary cl-auth-guard-link">
            Go to Login
          </a>
        </div>
      </div>
